...
export const RentalDetailModal: React.FC<RentalDetailModalProps> = ({
  ...
  const handleConfirmPayment = async (e: React.FormEvent) => {
    ...
    try {
      await api.rentals.payPayment(rental.id, payingPayment.id, {
        formaPagamento: paymentMethod,
        dataPagamento: paymentDate,
        observacoes: paymentNotes,
      });

      toastSuccess('Pagamento registrado!', `Parcela #${payingPayment.numeroParcela} marcada como PAGA.`);
    ...
    <>
      ...
      {payingPayment && (
        <Modal
          ...
          description={`Valor: ${maskCurrency(payingPayment.valor)} | Vencimento: ${formatDate(payingPayment.dataVencimento)}`}
          maxWidth="md"
        >
